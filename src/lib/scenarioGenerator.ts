...
// scenarios (client → project → scrapingPlan → scrapping option → seeds) from the
...
// (scrapingPlan.project = project.name, scrapingPlan.scrappingOption = option.name).
...
import { getProjects, PROJECTS_KEY, typeFromName, type Project, type AssignedScrapingPlan } from "./projects";
import { getScrapingPlans, SCRAPING_PLANS_KEY, emptyScrapingPlan, subProjects, type ScrapingPlan } from "./scrapingPlans";
 ...
 *  these types so each scrapingPlan's seeds are valid for its scrapping option. */
...
// samples + clones these so fabricated scrapingPlans carry REAL seed descriptions,
...
// Seed volume per scrapingPlan↔seeds relation for a single-client DEEP generate —
...
// `dup` (the pool wrapped past its length) appends a counter so within-scrapingPlan
...
// scrapingPlan can carry hundreds of type-correct seeds.
const buildSubSeeds = (types: SeedType[], count: number, store: string, slug: string, salt: number, fromDiscovery = false): Seed[] => {
  ...
  types.forEach((type, ti) => {     // keep each scrapingPlan's seed descriptions distinct.
...
// A geolocated (MANUAL) scrapingPlan scrapes every location of its store, so the
...
const locationSetLabel = (store: string): string => {
  ...
  // MANUAL scrapingPlan references a set whose locations the UI can actually show.
...
export function buildScenario(clientSlug: string, jobs: RealJob[], seedsPerSub: number = SEEDS_PER_SUB): BuiltScenario {
  ...
  const scrapingPlans: ScrapingPlan[] = [];
  const seeds: Seed[] = [];
  const assigned: AssignedScrapingPlan[] = [];
  ...
  jobs.forEach((job, ji) => {
    ...
    // --- the seeds for this scrapingPlan: hundreds of seeds whose TYPES are valid
    ...
    // Frequency lives on the scrapping option now; the scrapingPlan carries the new Selection parameters.
    ...
    const sub: ScrapingPlan = {
      ...emptyScrapingPlan(), id: uid(), name: job.name, projects: [project.name], store: job.store,
    ...
    // --- discovery (PLP/MEDIA) → create a sibling PDP scrapingPlan + destinationOption
    if (isDiscovery(job.extractionType)) {
      ...
      // same store) map to the SAME PDP sibling name — disambiguate so scrapingPlan and
      ...
      const pdpSub: ScrapingPlan = {
        ...emptyScrapingPlan(), id: uid(), name: pdpOptName, projects: [project.name], store: job.store,
      ...
      scrappingOptions.push(pdpOption); scrapingPlans.push(pdpSub); seeds.push(...pdpSeeds);
    ...
    scrappingOptions.push(option); scrapingPlans.push(sub); seeds.push(...subSeeds);
    assigned.push({ id: sub.id, name: sub.name, store: sub.store, geo: sub.geo, type: typeFromName(sub.name), expiration: "-" });
  });

  project.assignedScrapingPlans = assigned;
  return { slug: clientSlug, client, clientIsNew, project, scrappingOptions, scrapingPlans, seeds };
}
...
export function persistScenario(b: BuiltScenario): void {
  ...
  // A valid scenario is client → project → at least ONE scrapingPlan (each with its
  // 1:1 scrapping option). Never persist a project that has no scrapingPlan — that is
  // exactly the invalid use case we guard against.
  if (!b.scrapingPlans.length) return;
  ...
  // orphaned project without its scrapingPlans. On failure we rethrow so the caller
  // (Generate / Generate-all) records it and warns — with no half-written scenario.
  const KEYS = [CLIENTS_KEY, PROJECTS_KEY, SCRAPING_PLANS_KEY, SCRAPPING_OPTIONS_KEY, SEEDS_KEY, SIM_KEY];
  ...
  try {
    ...
    write(SCRAPING_PLANS_KEY, [...getScrapingPlans(), ...b.scrapingPlans]);
    ...
    ix.batches.push({
      ...
      subIds: b.scrapingPlans.map((s) => s.id),
...
/** Remove desk-test projects (`bom = "SIMUL"`) that have NO scrapingPlan referencing
 *  them — the invalid "project without a scraping plan" use case, whether left behind
...
export function pruneInvalidSimProjects(): number {
  ...
  const usedProjectNames = new Set(getScrapingPlans().flatMap(subProjects));
...
type ClearCounts = { projects: number; scrapingPlans: number; scrappingOptions: number; seeds: number; clients: number };
...
function removeBatches(batches: SimBatch[]): ClearCounts {
  ...
  write(SCRAPING_PLANS_KEY, getScrapingPlans().filter((s) => !subIds.has(s.id)));
  ...
  return { projects: projIds.size, scrapingPlans: subIds.size, scrappingOptions: scrapNames.size, seeds: seedIds.size, clients: createdClients.size };
}

/** Remove EVERYTHING the simulator created; leaves seeded/real data intact. */
export function clearSimulated(): ClearCounts {
  if (typeof window === "undefined") return { projects: 0, scrapingPlans: 0, scrappingOptions: 0, seeds: 0, clients: 0 };
...
// A geolocated (MANUAL) scrapingPlan extracts each of its store's active locations, so
...
export function estimateTasks(sub: ScrapingPlan, opt?: ScrappingOptionValues, storeLocCount?: number): number {
...
export function validateScenario(b: BuiltScenario): string[] {
  ...
  for (const s of b.scrapingPlans) {
